  ...
  return {
    async handle(req, res, endpoint, data, user, admin, send) {
      ...
      if (endpoint.startsWith('/__local/verification-image/')) {
        ...
        const response = await media.response(
          'evidence',
          'verification/' + row.file,
        );
          ...
          send(200, {
            requests: store
              ...
              .map((r) => ({
                ...
                image: '/api/app/verification-image/' + r.id,
              }))
      ...
      const id = randomUUID(),
        file = id + '.' + ext;
      await media.put('evidence', 'verification/' + file, bytes, data.type);
